  ...
  return (
    <li className="onhover-dropdown">
      <div
        ...
        onClick={() =>
          setIsComponentVisible(prev => (prev !== 'notification' ? 'notification' : ''))
        }
      >
        <RiNotificationLine />
        {notification?.length > 0 && (
          <span className="badge rounded-pill badge-theme">{notification.length}</span>
        )}
      </div>
      <ul
        className={`notification-dropdown onhover-show-div ${isComponentVisible == 'notification' ? 'active' : ''}`}
      >
        <li>
          <RiNotificationLine />
