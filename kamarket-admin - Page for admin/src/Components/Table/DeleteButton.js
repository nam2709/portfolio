...
async function deleteCategory({ CategoryId }) {
  const categoryId = CategoryId.replace(/\./g, '#SUB');
  try {
    const token = await fetchAuthSession()
      .then(session => session?.tokens?.idToken?.toString())
      .catch(() => null)
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/category/${categoryId}?type=original`, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`,
...
const DeleteButton = ({ obj, noImage, refetch }) => {
  ...
  const handleClick = async () => {
    ...
    } else if (obj?.productId && obj?.product_thumbnail) {
