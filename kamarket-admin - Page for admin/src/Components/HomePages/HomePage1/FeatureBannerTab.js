...
const FeatureBannerTab = ({ values, setFieldValue, productData, categoryData, setSearch }) => {
  ...
  const removeBanners = index => {
    if (values['content']['featured_banners']['banners'].length > 1) {
      let filterValue = values['content']['featured_banners']['banners'].filter(
        (item, i) => i !== index
      )
      setFieldValue('[content][featured_banners][banners]', filterValue)
      filterValue?.forEach((elem, i) => {
        elem?.image_url &&
          setFieldValue(`featureBannerImage${i}`, { original_url: elem?.image_url })
        elem?.redirect_link?.link_type &&
          setFieldValue(`featureRedirectLinkType${i}`, elem?.redirect_link?.link_type)
        elem?.redirect_link?.link &&
          setFieldValue(`featureRedirectLink${i}`, elem?.redirect_link?.link)
      })
    }
  }
  return (
    <>
      {
        <Btn
          ...
          onClick={() =>
            setFieldValue('[content][featured_banners][banners]', [
              ...values['content']['featured_banners']['banners'],
              { title: '', description: '' },
            ])
          }
          title="AddBanner"
              ...
              <div className="shipping-accordion-custom">
                ...
                {active == index && (
                  <div className="rule-edit-form">
                    <FileUploadField
                      name={`featureBannerImage${index}`}
                      title="Image"
                      id={`featureBannerImage${index}`}
                      type="file"
                      values={values}
                      setFieldValue={setFieldValue}
                      showImage={values[`featureBannerImage${index}`]}
                    ...
                    <CommonRedirect
                      ...
                      nameList={{
                        selectNameKey: `featureRedirectLinkType${index}`,
                        multipleNameKey: `featureRedirectLink${index}`,
                      }}
                      setSearch={setSearch}
                    />
                    <CheckBoxField
                      name={`[content][featured_banners][banners][${index}][status]`}
                      title="Status"
                    />
                  </div>
                )}
