...
const BankOfferTab = ({ values, setFieldValue }) => {
  ...
  const removeBanners = index => {
    if (values['content']['bank_wallet_offers']['offers'].length > 1) {
      let filterValue = values['content']['bank_wallet_offers']['offers'].filter(
        (item, i) => i !== index
      )
  ...
  return (
    <>
      <SimpleInputField
        nameList={[
          {
            name: `[content][bank_wallet_offers][title]`,
      ...
      <Btn
        ...
        onClick={() =>
          setFieldValue('[content][bank_wallet_offers][offers]' || [], [
            ...values['content']?.['bank_wallet_offers']?.['offers'],
            { title: '', description: '' },
          ])
        }
        title="Add Offer"
              ...
              <div className="shipping-accordion-custom">
                ...
                {active == index && (
                  <div className="rule-edit-form">
                    <FileUploadField
                      name={`bankOfferImage${index}`}
                      showImage={values[`bankOfferImage${index}`]}
                      title="Image"
                      id={`bankOfferImage${index}`}
                      type="file"
                      values={values}
                    ...
                    <SimpleInputField
                      nameList={[
                        {
                          name: `[content][bank_wallet_offers][offers][${index}][coupon_code]`,
                    ...
                    />
                    <CheckBoxField
                      name={`[content][bank_wallet_offers][offers][${index}][status]`}
                      title="Status"
                    />
                  </div>
                )}
