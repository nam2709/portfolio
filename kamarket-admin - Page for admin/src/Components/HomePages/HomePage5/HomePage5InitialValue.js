const HomePage5InitialValue = data => {
  ...
  data?.content?.bank_wallet_offers?.offers.length > 0 &&
    data?.content?.bank_wallet_offers?.offers.forEach((elem, index) => {
      elem.bank_image_url
        ? (obj2[`bankOfferBankImage${index}`] = { original_url: elem.bank_image_url })
        : ''
      return obj2
    })
  ...
  return {
    ...
    // Images
    productDealsImage: data?.content?.product_with_deals?.deal_of_days?.image_url
      ? { original_url: data?.content?.product_with_deals?.deal_of_days?.image_url }
      : '',

    homeBannerImage: data?.content?.home_banner?.main_banner?.image_url
