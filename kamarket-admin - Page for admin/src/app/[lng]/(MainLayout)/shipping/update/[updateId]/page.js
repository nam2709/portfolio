...
const AddShippingRules = ({ params }) => {
  ...
  const { data, isLoading, refetch } = useQuery(
    [`${shipping}/update/${updateId}`],
    () => request({ url: `${shipping}/${updateId}` }),
    {
      enabled: false,
  ...
  const { mutate: createMutate, isLoading: createMutateIsloading } = useMutation(
    data => request({ url: shippingRule, data, method: 'post' }),
    {
      onSuccess: resData => {
        SuccessHandle(
          resData,
  ...
  const { mutate: updateMutate, isLoading: updateLoading } = useMutation(
    data => request({ url: shippingRule + '/' + active, method: 'put', data }),
    {
      onSuccess: resData => {
        SuccessHandle(
          resData,
