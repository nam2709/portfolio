...
export function ProductInitValues(oldData, updateId) {
  ...
  return {
    ...
    variation_image_id: '',
    // Setup
    tags: updateId ? oldData?.tags?.map(item => item) || [] : [],
    categories: updateId ? oldData?.categories || '' : '',
    ...
    ship_express: updateId ? Boolean(Number(oldData?.ship_express)) : '',
