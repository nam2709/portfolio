  ...
  const fetchAndFilterCategories = async () => {
    ...
    try {
      ...
      if (token) {
        ...
        // Filter categories that don't contain "#SUB" in their categoryId
        const filteredCategories = categories.categories.Items.filter(
          category => !category?.categoryId?.includes("#SUB")
        );

        // Map the filtered categories to fetch the corresponding category data
        const categoryItemsPromises = filteredCategories.map(async category => {
        ...
        const validCategoryItems = categoryItems.filter(item => item && item.length !== 0);
        console.log('categoryItems',validCategoryItems)
        setFilteredCategories(validCategoryItems);
      }
    } catch (error) {
