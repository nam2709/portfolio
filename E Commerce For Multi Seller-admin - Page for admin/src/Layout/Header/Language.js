...
const Language = ({ isComponentVisible, setIsComponentVisible }) => {
  ...
  const langData = [
    { LanuageName: 'Tiếng Việt', lang: 'vi', icon: 'vn' },
  ...
  const splitPathname = isLangIncludes ? pathname.split(isLangIncludes.lang)[1] : ''
  useEffect(() => {
    ...
    <li className="profile-nav onhover-dropdown">
      ...
      >
        {langData?.map((data, i) => (
          ...
          >
            <Link href={`/${data.lang}/${splitPathname}${searchParams ? '?' + searchParams : ''}`}>
