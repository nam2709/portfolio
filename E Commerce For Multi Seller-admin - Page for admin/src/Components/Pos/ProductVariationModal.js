            ...
            <div className="selection-section mt-4" key={index}>
              ...
              <ul>
                {elem?.attribute_values?.map((item, i) => (
                  <li key={i}>
                    <Btn
                      className={`${cloneVariation?.variantIds?.includes(item?.id) ? 'active' : ''}`}
                      disabled={!cloneVariation?.attributeValues?.includes(item?.id)}
                      onClick={() => setVariant(cloneVariation?.product?.variations, item)}
                    >
                      {item?.value}
                    </Btn>
                  </li>
