  ...
  const isHandelEdit = (e, tableData, headerData) => {
    ...
    if (!headerData.noEdit) {
      if (headerData?.optionHead?.type == 'View') {
        ...
        tableData?.id &&
          router.push(`/${i18Lang}/${moduleName.toLowerCase()}/update/${tableData.id}`)
      }
    }
  }
      ...
      <thead>
        <tr>
          <>
            ...
            {headerData?.isShipping && <th className="sm-width">{t('Shipping')}</th>}
        ...
        {headerData?.data.length > 0 ? (
          headerData?.data?.map((tableData, index) => (
            <tr key={index}>
              ...
              <>
                {headerData?.column.map((item, i) => (
                  <td
                    ...
                    key={i}
                    onClick={e => item.type !== 'switch' && isHandelEdit(e, tableData, headerData)}
                  >
                    { item.render ? (
                      item.render(tableData)
                    ) : item.type == 'date' ? (
                      ...
                      <>
                        {!edit || headerData?.data?.[index].system_reserve == '1' ? (
                          <Status data={tableData} refetch={refetch} url={url} disabled={true} />
                        ) : (
                          <Status
                            data={tableData}
                            refetch={refetch}
              ...
              {headerData?.isShipping && (
                <td className="sm-width">
                  <button onClick={() => handleShip(tableData)} style={{ background: 'transparent', border: 'none', padding: 0}}>
