...
const HomePage5Submit = values => {
  ...
  if (values['productDealProductIds']) {
    values['content']['product_with_deals']['products_list']['product_ids'] =
      values['productDealProductIds']
  }
  ...
  values['content']['product_with_deals']['deal_of_days']['deals'].forEach((elem, index) => {
    if (values[`DealOfDaysProduct${index}`]) {
      values['content']['product_with_deals']['deal_of_days']['deals'][index]['product_id'] =
        values[`DealOfDaysProduct${index}`]
    }
  })

  // Passing images
  if (values['productDealsImage']) {
    values['content']['product_with_deals']['deal_of_days']['image_url'] =
      values['productDealsImage'].original_url
  ...
  values['content']['bank_wallet_offers']['offers'].forEach((elem, i) => {
    if (values[`bankOfferImage${i}`]) {
      values['content']['bank_wallet_offers']['offers'][i]['image_url'] =
        values[`bankOfferImage${i}`].original_url
    } else values['content']['bank_wallet_offers']['offers'][i]['image_url'] = ''
  })

  values['content']['bank_wallet_offers']['offers'].forEach((elem, i) => {
    if (values[`bankOfferBankImage${i}`]) {
      values['content']['bank_wallet_offers']['offers'][i]['bank_image_url'] =
        values[`bankOfferBankImage${i}`].original_url
